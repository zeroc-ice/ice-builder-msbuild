...
directory to the ClCompile `AdditionalIncludeDirectories` metadata, and more.
...
With most C++ projects, you need to add yourself the directory where `slice2cpp` outputs
header files to ClCompile's AdditionalIncludeDirectories. With the default settings, these
header files are generated in `$(IntDir)` and you would add:
```
<ItemDefinitionGroup>
    <ClCompile>
